...
When a question spans multiple domains, prefer the skill whose description matches the primary API surface, then load a second skill only if needed. Ownership notes at the bottom of each domain `SKILL.md` list related skills. Repo skill index: [`docs/SKILLS.md`](../../../SKILLS.md#cesiumjs-domain-skills).
