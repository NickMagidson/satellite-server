...
When a question spans multiple domains, prefer the skill whose description matches the primary API surface, then load a second skill only if needed. Ownership notes at the bottom of each domain `SKILL.md` list related skills. For Satellite Server tasks, also follow `docs/SKILLS.md` (CesiumJS domain skills section) and `docs/skills/SKILL-CESIUM-VIEWER-CHANGE.md` for app integration.
